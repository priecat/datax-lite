...
public class JobContainer extends AbstractContainer {
    ...
    /**
     * reader和writer jobContainer的实例（仅 preCheck/dryRun 单 content 路径使用）
     */
    ...
    /**
     * 支持多 content（多表）任务：每个 content 独立初始化 reader/writer 的 Job 实例并切分，
     * 切分结果合并回同一个 job.content，全部 task 共享同一套调度与 channel 池并发执行
     */
    private final List<String> readerPluginNames = new ArrayList<String>();

    private final List<Reader.Job> jobReaders = new ArrayList<Reader.Job>();

    private final List<String> writerPluginNames = new ArrayList<String>();

    private final List<Writer.Job> jobWriters = new ArrayList<Writer.Job>();
    ...
    private void init() {
        ...
        List<Configuration> contents = this.configuration.getListConfiguration(
                CoreConstant.DATAX_JOB_CONTENT);
        if (contents == null || contents.isEmpty()) {
            throw DataXException.asDataXException(
                    FrameworkErrorCode.CONFIG_ERROR, "job.content 不能为空");
        }
        ...
        for (int i = 0; i < contents.size(); i++) {
            //必须先Reader ，后Writer
            this.jobReaders.add(this.initJobReader(jobPluginCollector, i));
            this.jobWriters.add(this.initJobWriter(jobPluginCollector, i));
        }
    }

    private void prepare() {
        for (int i = 0; i < this.jobReaders.size(); i++) {
            this.prepareJobReader(i);
            this.prepareJobWriter(i);
        }
    }
     ...
     * 支持多 content（多表）：逐个 content 切分后合并进同一个 job.content，
     * taskId 全局递增保证唯一，所有表的 task 进入同一调度并发执行
     */
    private int split() {
        this.adjustChannelNumber();
        ...
        // 多表时按表均分总 channel 数，保证每张表至少切出 1 个 task
        int adviceNumber = Math.max(1,
                (this.needChannelNumber + contentNumber - 1) / contentNumber);

        List<Configuration> contentConfig = new ArrayList<Configuration>();
        int taskId = 0;
        for (int i = 0; i < contentNumber; i++) {
            List<Configuration> readerTaskConfigs = this.doReaderSplit(i,
                    adviceNumber);
            int taskNumber = readerTaskConfigs.size();
            List<Configuration> writerTaskConfigs = this.doWriterSplit(i,
                    taskNumber);

            List<Configuration> transformerList = contents.get(i)
                    .getListConfiguration(CoreConstant.JOB_TRANSFORMER);

            LOG.debug("content[{}] transformer configuration: {}",
                    i, JSON.toJSONString(transformerList));
            /**
             * 输入是reader和writer的parameter list，输出是content下面元素的list
             */
            List<Configuration> partContentConfig = mergeReaderAndWriterTaskConfigs(
                    readerTaskConfigs, writerTaskConfigs, transformerList,
                    this.readerPluginNames.get(i), this.writerPluginNames.get(i),
                    taskId);
            taskId += partContentConfig.size();
            contentConfig.addAll(partContentConfig);
            LOG.info("DataX content[{}] [{} -> {}] splits to [{}] tasks.",
                    i, this.readerPluginNames.get(i), this.writerPluginNames.get(i),
                    partContentConfig.size());
        }
    ...
    private void post() {
        for (int i = 0; i < this.jobWriters.size(); i++) {
            this.postJobWriter(i);
            this.postJobReader(i);
        }
    }

    private void destroy() {
        for (Writer.Job writer : this.jobWriters) {
            try {
                writer.destroy();
            } catch (Throwable e) {
                LOG.warn("DataX Writer.Job destroy failed.", e);
            }
        }
        for (Reader.Job reader : this.jobReaders) {
            try {
                reader.destroy();
            } catch (Throwable e) {
                LOG.warn("DataX Reader.Job destroy failed.", e);
            }
        }
        this.jobWriters.clear();
        this.jobReaders.clear();
    ...
    /**
     * reader job的初始化，返回Reader.Job。多 content 时按索引读取 job.content[i] 的配置
     */
    private Reader.Job initJobReader(JobPluginCollector jobPluginCollector, int index) {
        String readerNamePath = String.format("%s[%d].%s",
                CoreConstant.DATAX_JOB_CONTENT, index, CoreConstant.JOB_READER_NAME);
        String readerParameterPath = String.format("%s[%d].%s",
                CoreConstant.DATAX_JOB_CONTENT, index, CoreConstant.JOB_READER_PARAMETER);
        String writerParameterPath = String.format("%s[%d].%s",
                CoreConstant.DATAX_JOB_CONTENT, index, CoreConstant.JOB_WRITER_PARAMETER);

        String readerPluginName = this.configuration.getString(readerNamePath);
        classLoaderSwapper.setCurrentThreadClassLoader(LoadUtil.getJarLoader(
        ...
        classLoaderSwapper.restoreCurrentThreadClassLoader();
        this.readerPluginNames.add(readerPluginName);
        return jobReader;
    }

    /**
     * writer job的初始化，返回Writer.Job。多 content 时按索引读取 job.content[i] 的配置
     */
    private Writer.Job initJobWriter(JobPluginCollector jobPluginCollector, int index) {
        String writerNamePath = String.format("%s[%d].%s",
                CoreConstant.DATAX_JOB_CONTENT, index, CoreConstant.JOB_WRITER_NAME);
        String writerParameterPath = String.format("%s[%d].%s",
                CoreConstant.DATAX_JOB_CONTENT, index, CoreConstant.JOB_WRITER_PARAMETER);
        String readerParameterPath = String.format("%s[%d].%s",
                CoreConstant.DATAX_JOB_CONTENT, index, CoreConstant.JOB_READER_PARAMETER);

        String writerPluginName = this.configuration.getString(writerNamePath);
        classLoaderSwapper.setCurrentThreadClassLoader(LoadUtil.getJarLoader(
        ...
        jobWriter.setPeerPluginName(this.readerPluginNames.get(index));
        jobWriter.setJobPluginCollector(jobPluginCollector);
        jobWriter.init();
        classLoaderSwapper.restoreCurrentThreadClassLoader();

        this.writerPluginNames.add(writerPluginName);
        return jobWriter;
    }

    private void prepareJobReader(int index) {
        String readerPluginName = this.readerPluginNames.get(index);
        classLoaderSwapper.setCurrentThreadClassLoader(LoadUtil.getJarLoader(
                PluginType.READER, readerPluginName));
        LOG.info(String.format("DataX Reader.Job [%s] do prepare work .",
                readerPluginName));
        this.jobReaders.get(index).prepare();
        classLoaderSwapper.restoreCurrentThreadClassLoader();
    }

    private void prepareJobWriter(int index) {
        String writerPluginName = this.writerPluginNames.get(index);
        classLoaderSwapper.setCurrentThreadClassLoader(LoadUtil.getJarLoader(
                PluginType.WRITER, writerPluginName));
        LOG.info(String.format("DataX Writer.Job [%s] do prepare work .",
                writerPluginName));
        this.jobWriters.get(index).prepare();
        classLoaderSwapper.restoreCurrentThreadClassLoader();
    }

    // TODO: 如果源头就是空数据
    private List<Configuration> doReaderSplit(int index, int adviceNumber) {
        String readerPluginName = this.readerPluginNames.get(index);
        classLoaderSwapper.setCurrentThreadClassLoader(LoadUtil.getJarLoader(
                PluginType.READER, readerPluginName));
        List<Configuration> readerSlicesConfigs =
                this.jobReaders.get(index).split(adviceNumber);
        ...
        LOG.info("DataX Reader.Job [{}] splits to [{}] tasks.",
                readerPluginName, readerSlicesConfigs.size());
        classLoaderSwapper.restoreCurrentThreadClassLoader();
        return readerSlicesConfigs;
    }

    private List<Configuration> doWriterSplit(int index, int readerTaskNumber) {
        String writerPluginName = this.writerPluginNames.get(index);
        classLoaderSwapper.setCurrentThreadClassLoader(LoadUtil.getJarLoader(
                PluginType.WRITER, writerPluginName));

        List<Configuration> writerSlicesConfigs = this.jobWriters.get(index)
        ...
        LOG.info("DataX Writer.Job [{}] splits to [{}] tasks.",
                writerPluginName, writerSlicesConfigs.size());
        classLoaderSwapper.restoreCurrentThreadClassLoader();
     ...
     * @param readerPluginName   该 content 的 reader 插件名
     * @param writerPluginName   该 content 的 writer 插件名
     * @param startTaskId        多 content 合并时的 taskId 起始偏移，保证全局唯一
     * @return
     */
    private List<Configuration> mergeReaderAndWriterTaskConfigs(
            ...
            String readerPluginName, String writerPluginName, int startTaskId) {
        ...
        for (int i = 0; i < readerTasksConfigs.size(); i++) {
            ...
            taskConfig.set(CoreConstant.TASK_ID, startTaskId + i);
    ...
    private void postJobReader(int index) {
        String readerPluginName = this.readerPluginNames.get(index);
        classLoaderSwapper.setCurrentThreadClassLoader(LoadUtil.getJarLoader(
                PluginType.READER, readerPluginName));
        LOG.info("DataX Reader.Job [{}] do post work.",
                readerPluginName);
        this.jobReaders.get(index).post();
        classLoaderSwapper.restoreCurrentThreadClassLoader();
    }

    private void postJobWriter(int index) {
        String writerPluginName = this.writerPluginNames.get(index);
        classLoaderSwapper.setCurrentThreadClassLoader(LoadUtil.getJarLoader(
                PluginType.WRITER, writerPluginName));
        LOG.info("DataX Writer.Job [{}] do post work.",
                writerPluginName);
        this.jobWriters.get(index).post();
        classLoaderSwapper.restoreCurrentThreadClassLoader();
    }
